...
- **动态 STUN 直连与凭据记忆 (301: 前缀)**：针对公网 IP 和端口会随 STUN 变动的直连服务端，支持在客户端输入以 `301:` 开头的公网 HTTP(S) 地址（例如 `301:http://example.com/stun_ip`）。客户端连接时会自动获取最新的真实 `ip:port` 进行直连，并持久化绑定该条目的连接密码和操作系统登录凭据，IP 变动后无需重新配置或再次输入密码。
