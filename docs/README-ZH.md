...
### 优化特性 (Mod 改版功能)
本项目针对平板生产力与网络直连进行了深度优化：
- **动态 STUN 直连与凭据记忆 (302: 前缀 / HTTP(S) 地址)**：针对公网 IP 和端口会随 STUN 变动的直连服务端，支持在客户端输入以 `302:`（或兼容 `301:`）开头的地址，亦可直接输入 `http://` 或 `https://` 开头的地址（例如 `http://example.com/stun_ip`）。连接时支持自动解析 HTTP 301/302 重定向头（Location）提取真实直连地址，也支持从响应体中解析，并自动持久化绑定该条目的连接密码和操作系统登录凭据，IP 变动后无需重新配置或再次输入密码。
...
- **持续后台连接**：实现了 Android 前台服务以防止连接超时。即使应用最小化或切换到后台，连接也能保持活跃。
